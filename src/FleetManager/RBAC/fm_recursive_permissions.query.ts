export const TRUE_RECURSIVE_RELATION_QUERY = `
  WITH RECURSIVE parent_modules AS (
    SELECT rm.*
    FROM fm_relation_module rm
    WHERE rm.child_module_id = $1

    UNION

    SELECT rm2.*
    FROM fm_relation_module rm2
  ...
  all_relations AS (
    ...
    UNION
    SELECT * FROM fm_relation_module 
...
export const FALSE_RECURSIVE_RELATION_QUERY = `WITH RECURSIVE all_children AS (
  SELECT * FROM fm_relation_module WHERE parent_module_id = $1
...
UNION
SELECT id FROM fm_relation_module WHERE parent_module_id = $1;
